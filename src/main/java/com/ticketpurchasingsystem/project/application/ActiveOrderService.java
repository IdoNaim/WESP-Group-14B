...
public class ActiveOrderService implements IActiveOrderService {
    ...
    public ActiveOrderItem createPendingOrder(SessionToken sessionToken, String userId, String eventId){
        ...
        validateSession(sessionToken, "Session validation failed while creating pending order for userId: " + userId, "the session has ended");
        validateUserMatch(sessionToken, userId);
    ...
    public void addStandingAreaToActiveOrder(SessionToken sessionToken, String orderId, String areaId, int quantity) {
        ...
        try {
            ...
            activeOrderPublisher.publishReleaseStandingArea(sessionToken.getToken(), order.getEventId(), areaId, quantity);
            logger.error("got DB error when trying to update order: " + orderId + " with "+ quantity +" tickets for area "+ areaId + " of event "+ order.getEventId());
        }
    }
    ...
    public List<BarcodeDTO> completeOrder(IPaymentGateway paymentGateway, SessionToken sessionToken, PaymentDetails paymentDetails, String orderId){
        ...
        activeOrderHandler.validateOrderOwnership(currentTokenUser, order, "Unauthorized: Order does not belong to the current user");
    ...
    public void updateActiveOrder(SessionToken sessionToken, ActiveOrderDTO newOrderDTO){
        ...
        for (Map.Entry<String, Integer> entry : standingToReserve.entrySet()) {
            standingReserved = activeOrderPublisher.publishReserveStandingArea(sessionToken.getToken(), order.getEventId(), entry.getKey(), entry.getValue());
            if (!standingReserved) {
                break;
            } else {
        ...
        for (Map.Entry<String, Integer> entry : standingAreaTicketsToRelease.entrySet()) {
            activeOrderPublisher.publishReleaseStandingArea(sessionToken.getToken(), order.getEventId(), entry.getKey(), entry.getValue());
        }
    ...
    private void validateUserMatch(SessionToken token, String userId) {
        ...
        if (!tokenUser.equals(userId)) {
            logger.error("Identity discrepancy match failed: userId " + userId + " does not match token context holder " + tokenUser);
    ...
    private void rollbackOrderReservations(String sessionToken, ActiveOrderDTO order) {
        ...
        if (activeOrderHandler.canReleaseSeats(order.getSeatIds())) {
            activeOrderPublisher.publishReleaseSeats(sessionToken, order.getOrderId(), order.getEventId(), order.getSeatIds());
        }
